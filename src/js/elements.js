module.exports = function(lib, xelib, helpers) {
    let { applyEnums, elementContext, arrayItemContext, Fail, GetArray,
        GetBool, GetEnumValue, GetHandle, GetInteger, GetStringArray,
        wcb, buildOptions } = helpers;
    ...
    Object.assign(xelib, {
        ...
        GetElements: function(id = 0, path = '') {
            let {sort, filter, sparse} = buildOptions(arguments, 2, {
                sort: false,
                filter: false,
                sparse: false
            });
            return GetArray(function(_len) {
                if (!lib.GetElements(id, wcb(path), sort, filter, sparse, _len))
        ...
        SetLinksTo: function(id, id2, path = '') {
            // swap id2 and path if called with old argument order
            if (typeof id2 === 'string') [id2, path] = [path, id2];
