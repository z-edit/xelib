module.exports = function(lib, xelib, helpers) {
    ...
    Object.assign(xelib, {
        ...
        BuildReferenceMap: function(
            id,
            search = '',
            keyFn = xelib.EditorID,
            valueFn = null
        ) {
            let records = xelib.GetRecords(id, search);
            return records.reduce((obj, rec) => {
                let key = keyFn(rec);
                if (key) obj[key] = valueFn ? valueFn(rec) : rec;
                return obj;
            }, {});
        },
        ResolveElements: function(id, paths) {
            return Object.keys(paths).reduce((elements, name) => {
                let path = paths[name];
                elements[name] = xelib.GetElement(id, path);
                return elements;
            }, {});
        },
        ResolveElementsEx: function(id, paths) {
            return Object.keys(paths).reduce((elements, name) => {
                let path = paths[name];
                elements[name] = xelib.GetElementEx(id, path);
                return elements;
            }, {});
        }
    });
