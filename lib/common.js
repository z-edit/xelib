module.exports = function(lib, xelib) {
    ...
    xelib.GetBodyTemplate = function(record) {
        return xelib.GetElement(record, 'BODT') ||
            xelib.GetElement(record, 'BOD2');
    };

    xelib.GetArmorType = function(record) {
        return xelib.WithHandle(xelib.GetBodyTemplate(record), bodyTemplate => {
            return xelib.GetValue(bodyTemplate, 'Armor Type');
        });
    };

    xelib.SetArmorType = function(record, armorType) {
        xelib.WithHandle(xelib.GetBodyTemplate(record), bodyTemplate => {
            xelib.SetValue(bodyTemplate, 'Armor Type', armorType);
        });
    };
